...
func (c AddItemAdminCommand) ToCommand() string {
	return fmt.Sprintf("additem %q %q %d", c.Username, c.ItemType, c.Count)
}

func (c AddItemAdminCommand) ParseResponse(response string) (string, error) {
	switch response {
	case fmt.Sprintf("Item %s Added in %s's inventory.", c.ItemType, c.Username):
		return "", nil
	case "No such user":
	...
	case fmt.Sprintf("Item %s doesn't exist.", c.ItemType):
		return "", &domainerror.DomainError{
