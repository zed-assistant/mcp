...
func (c AddXPAdminCommand) ToCommand() string {
	return fmt.Sprintf("addxp %q %s=%d", c.Username, c.Perk, c.Amount)
}

func (c AddXPAdminCommand) ParseResponse(response string) (string, error) {
	switch {
	case response == fmt.Sprintf("Added %d.0 %s xp's to %s", c.Amount, c.Perk, c.Username):
		return "", nil
	case response == "No such user":
	...
	case strings.Contains(response, "List of available perks"):
		return "", &domainerror.DomainError{
